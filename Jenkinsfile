node{
    ...
    stage('git code checkout'){
        try{
            ...
            git 'https://github.com/Pratiksha-U/star-agile-insurance-project.git'
        }
        catch(Exception e){
    ...
    stage('Containerize the application'){
        ...
        sh "${dockerCMD} build -t tomjerryy/insurance-eta-app:${tagName} ."
    }
    
    stage('Pushing it ot the DockerHub'){
        echo 'Pushing the docker image to DockerHub'
        withCredentials([string(credentialsId: 'dock-password', variable: 'dockerHubPassword')]) {
        sh "${dockerCMD} login -u tomjerryy -p ${dockerHubPassword}"
        sh "${dockerCMD} push tomjerryy/insurance-eta-app:${tagName}"
